/**
 * tz-coords — IANA 时区数据（**用户所在地**专用，与 145 个旅行目的地数据集完全无关）。
 *
 * 数据来源（真实数据，非拟合）：
 *   IANA Time Zone Database（tztdb 主干）
 *   · zone1970.tab / zone.tab —— 每个时区的主城市参考坐标（度分 / 度分秒）
 *   · backward —— 历史别名 → 主时区（如 Asia/Chongqing → Asia/Shanghai）
 *   取数日期：2026-09-14
 *
 * 用途：把"用户所在地"解析成**城市级标签 + 可用坐标**，用于本地时间与本地天气。
 * 用户所在地**不要求**存在于 145 个旅行目的地中（例如 America/Chicago）。
 *
 * 约定：
 *   · 固定偏移时区（Etc/GMT-8、Etc/UTC …）已在生成时剔除：它们不是地点，
 *     不能用来取天气（这类时区只显示 Local time，Weather 显示不可用）。
 *   · 别名先解析到主时区，保证"城市标签"与"坐标"永远指向同一座城市。
 *   · 坐标精确到 2 位小数（≈1 km），足够城市级天气查询；纯静态数据、零网络请求。
 */

/** 主时区 → [lat, lon]。 */
export const TZ_COORDS: Readonly<Record<string, readonly [number, number]>> = {
...
/** 历史别名 → 主时区（先解析别名，再取坐标与城市名）。 */
export const TZ_ALIASES: Readonly<Record<string, string>> = {
...
/** 时区 → [lat, lon]；未知时区返回 null（调用方必须安全降级，不猜坐标）。 */
export function tzCoordsFor(timeZone: string | null | undefined): [number, number] | null {
  if (!timeZone) return null;
  const hit = TZ_COORDS[canonicalTimeZone(timeZone)];
